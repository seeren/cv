        ...
        {experiences.map((experience, index) => (
          <Fragment key={index}>
            <div className="pt-4 first:pt-0 border-t border-gray-200 first:border-t-0 text-gray-800 space-y-3">
              ...
              {experience.context && <p className="mt-2 text-[12px] text-gray-700">{experience.context}</p>}
                ...
                <div className="mt-4 space-y-4 pl-4 border-l-2 border-gray-200">
                  {experience.missions.map((mission, missionIndex) => (
                    <div key={missionIndex} className="space-y-2.5">
                      <div>
                        <h4 className="text-sm font-medium text-gray-800 mb-1.5">{mission.subtitle}</h4>
                      ...
                      {mission.responsabilities && mission.responsabilities.length > 0 && (
                        <div className="print:break-inside-avoid">
                          <div className="rounded border border-gray-100 bg-gray-50/30 print:bg-transparent">
                            <div className="p-1.5">
                              <h5 className="text-[9px] font-medium text-gray-500">Responsabilités</h5>
                            </div>
                            <ul className="px-2.5 space-y-0.5 text-[11px] text-gray-800 list-none">
                              {mission.responsabilities.map((responsability, i) => (
                      ...
                      {mission.achievements && mission.achievements.length > 0 && (
                        <div className="print:break-inside-avoid">
                          <div className="p-1.5">
                            <h5 className="text-[9px] font-medium text-gray-500">Réalisations</h5>
                          </div>
                          <ul className="px-2.5 space-y-0.5 text-[11px] text-gray-800">
                            {mission.achievements.map((achievement, i) => (
                              <li key={i} className="leading-snug hyphens-auto flex items-start gap-1.5">
                                <CheckCircle2
                                  className="mt-[2px] h-2.5 w-2.5 text-green-600/60 print:text-gray-700 shrink-0"
                                  aria-hidden
                                />
              ...
              <div className="print:break-inside-avoid">
                <div className="p-2">
                  <h4 className="text-[10px] font-medium text-gray-500">Réalisations</h4>
                </div>
              ...
              <div className="print:break-inside-avoid">
                {experience.responsabilities && experience.responsabilities.length > 0 && (
                  <div className="rounded border border-gray-100 bg-gray-50/40 print:bg-transparent">
                    <div className="p-2">
                      <h4 className="text-[10px] font-medium text-gray-500">Responsabilités</h4>
                    </div>
              ...
              <div>
                <h4 className="text-[10px] font-medium text-gray-500">Environnement</h4>
                {experience.environment && experience.environment.length > 0 && (
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {experience.environment.map((env, i) => (
                      <span key={i} className="text-[10px] px-1.5 py-0.5 rounded border border-gray-300 text-gray-600">
                        {env}
                      </span>
                    ))}
                  </div>
